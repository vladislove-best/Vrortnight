...
class player:
    ...
    def aiming(self):
        for i in range(len(weapon_list)):
            if abs(weapon_list[i].pos_y - self.pos_y) <45 and abs(weapon_list[i].pos_x - self.pos_x) < 40:
                self.mywepon = weapon_list[i]
                self.pos_y = self.mywepon.pos_y
                self.pos_x = self.mywepon.pos_x
                print(1)
                break
            print(2)
            # return self.pos_x, self.pos_y, self.mywepon
class bulets:
    ...
    def update(self):
        ...
        if sum(self.color_around) < 450 :
            ...
            if abs(player1.pos_y - self.pos_y) < self.size and abs(player1.pos_x - self.pos_x) < self.size :
                del player1 
                win()
            if abs(player2.pos_y - self.pos_y) < self.size and abs(player2.pos_x - self.pos_x) < self.size :
                del player2 
                win()
...
class weapon:
    ...
    def shoot(self):
        ...
        if self.counter > self.reloading_time:
            bulet_list.append(bulets(self.type,self.pos_x,self.pos_y, \
                self.bulet_speed,self.angl))
            self.counter = 0
pygame.init()
pygame.font.init()
...
player1 = player(635,display_heigth/2, 'a0000723242_16.jpg')
player2 = player(645,display_heigth/2, 'яблоко_стива_хуйса.jpg')
running = True
FPS = 60
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_a:
                if player1.mywepon != 'None':
                    player1.mywepon.angl -= 2
                    player1.mywepon.update()
                else:
                    player1.walk_left()
            if event.key == pygame.K_d:
                if player1.mywepon != 'None':
                    ...
                    player1.walk_right()                        
            if event.key == pygame.K_w and player1.mywepon == 'None':
                player1.walk_up()
            if event.key == pygame.K_s and player1.mywepon == 'None':
                print('пришёл')
                player1.walk_down()
                # player1.pos_x, player1.pos_y, player1.mywepon = player1.aiming()
                player1.aiming()
            if event.key == pygame.K_e and player1.mywepon != 'None':
                player1.mywepon.shoot()
            if event.key == pygame.K_q and player1.mywepon != 'None':
                print(3)
                player1.mywepon = 'None'
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                if player2.mywepon != 'None':
                    player2.mywepon.angl -= 2
                    player2.mywepon.update()
                else:
                    player2.walk_left()
            if event.key == pygame.K_RIGHT:
                if player2.mywepon != 'None':
                    ...
                    player2.walk_right()                        
            if event.key == pygame.K_UP and player2.mywepon == 'None':
                player2.walk_up()
            if event.key == pygame.K_DOWN and player2.mywepon == 'None':
                player2.walk_down()
                # player2.pos_x, player2.pos_y, player2.mywepon = player2.aiming()
                player2.aiming()
            if event.key == pygame.K_k and player2.mywepon != 'None':
                player2.mywepon.shoot()
            if event.key == pygame.K_l and player2.mywepon != 'None':
                player2.mywepon = 'None'
        if event.type == pygame.KEYUP:
            if player2.mywepon != 'None':
                pass
                # mywepon.angl = 0
